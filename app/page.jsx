...
// Sidebar Component
const Sidebar = ({ activeView, setActiveView, currentUser, notifications = [] }) => {
  const reviewNotifs = notifications.filter(n => n.type === 'review' && !n.read).length;
  
  const menuItems = [
    { id: 'dashboard', label: 'Dashboard', icon: Home },
    { id: 'tasks', label: 'Task Queue', icon: List },
    { id: 'projects', label: 'Projects', icon: Briefcase },
    { id: 'review', label: 'Review Queue', icon: CheckCircle, badge: reviewNotifs > 0 ? reviewNotifs : null },
...
// Header Component
const Header = ({ title, subtitle, notifications = [], setNotifications, setActiveView }) => {
                ...
                {notifications.length === 0 ? (
                  ...
                  notifications.map(notif => (
                    <div 
                      ...
                      onClick={() => {
                        markAsRead(notif.id);
                        if (notif.link && setActiveView) setActiveView(notif.link);
                        setShowNotifications(false);
                      }}
                      className={`p-3 border-b border-slate-50 cursor-pointer hover:bg-slate-50 transition-colors ${!notif.read ? 'bg-blue-50' : ''}`}
                    >
                      <p className={`text-sm ${!notif.read ? 'font-medium text-slate-900' : 'text-slate-600'}`}>
                        {notif.message}
                      </p>
                      <p className="text-xs text-slate-400 mt-1">{formatDate(notif.timestamp)}</p>
                    </div>
                  ))
                )}
              </div>
...
// Dashboard View
const DashboardView = ({ tasks, projects, activityLog = [], setActiveView }) => {
  ...
  const recentActivity = activityLog.slice(0, 5).map(item => {
    const iconMap = {
      'TASK_APPROVED': { icon: CheckCircle, color: 'emerald' },
      'TASK_DENIED': { icon: XCircle, color: 'rose' },
      'TASK_CREATED': { icon: Plus, color: 'blue' },
      'TASK_100_PERCENT': { icon: CheckSquare, color: 'violet' },
      'QAQC_BYPASSED': { icon: AlertTriangle, color: 'amber' },
      'QAQC_COMPLETED': { icon: Shield, color: 'emerald' },
      'AE_RELEASED': { icon: FileCheck, color: 'teal' },
      'CODE_CREATED': { icon: Layers, color: 'cyan' },
      'PROGRESS_UPDATE': { icon: RefreshCw, color: 'slate' },
    };
    const mapping = iconMap[item.action] || { icon: Activity, color: 'slate' };
    return {
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-4 gap-4">
        <div onClick={() => setActiveView && setActiveView('tasks')} className="cursor-pointer">
          <StatCard 
            title="Tasks in Queue" 
            value={tasks.filter(t => t.status !== 'Approved').length} 
            subtitle={`${tasks.filter(t => t.status === 'In Production').length} in production`}
            icon={List} 
            color="blue"
          />
        </div>
        <div onClick={() => setActiveView && setActiveView('review')} className="cursor-pointer">
          <StatCard 
        ...
        <div onClick={() => setActiveView && setActiveView('tasks')} className="cursor-pointer">
          <StatCard 
            ...
            icon={AlertTriangle} 
            color={overdueTasks.length > 0 ? 'rose' : 'green'}
          />
        </div>
        <StatCard 
          title="Completed" 
        ...
        <div className="col-span-2 bg-white rounded-xl shadow-sm border border-slate-200">
          <div className="p-5 border-b border-slate-100">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => setActiveView && setActiveView('activity')} className="text-sm text-blue-600 hover:text-blue-700 font-medium">View All</button>
            </div>
          </div>
              ...
              recentActivity.map((item, i) => (
                <div key={i} className="p-4 flex items-start gap-4 hover:bg-slate-50 transition-colors">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-slate-500 text-sm truncate">{item.task} • {item.user}</p>
                  </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-5">
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-bold text-slate-900">Project Status Overview</h3>
          <button onClick={() => setActiveView && setActiveView('projects')} className="text-sm text-blue-600 hover:text-blue-700 font-medium">View All</button>
        </div>
        <div className="grid grid-cols-4 gap-4">
...
// Task Queue View
const TaskQueueView = ({ tasks, setTasks, currentUser, projects, addActivity, addNotification }) => {
  ...
  const handleProgressUpdate = (taskId, newProgress) => {
    ...
    // Log activity for 100% completion
    if (newProgress === 100 && task && addActivity) {
      addActivity({
        jobPhase: `${task.job}.${task.phase}`,
        catCode: task.catCode,
        action: 'TASK_100_PERCENT',
        notes: `${task.title} marked 100% complete - ready for review`,
        taskId: task.id
      });
      if (addNotification) {
        addNotification({
          type: 'review',
          message: `${task.title} ready for review`,
          link: 'review'
        });
      }
    }
...
// Review Queue View
const ReviewQueueView = ({ tasks, setTasks, addActivity, addNotification }) => {
  const reviewTasks = tasks.filter(t => t.status === 'Review');
  ...
  const handleApproval = (type) => {
    ...
    const approvalLabels = {
      ...
      comments: 'Approved for Comments',
      close: 'Approved and Closed'
    };
    
    setTasks(prev => prev.map(t => {
      if (t.id === selectedTask.id) {
        return { ...t, status: 'Approved', approvalType: type, lastUpdated: new Date().toISOString().split('T')[0], priority: 0 };
      }
      return t;
    }));
    ...
    if (addActivity) {
      addActivity({
        jobPhase: `${selectedTask.job}.${selectedTask.phase}`,
        catCode: selectedTask.catCode,
        action: 'TASK_APPROVED',
        notes: `${selectedTask.title} - ${approvalLabels[type]}`,
  ...
  const handleDenial = () => {
    ...
    if (addActivity) {
      addActivity({
        ...
        catCode: selectedTask.catCode,
        action: 'TASK_DENIED',
        notes: `${selectedTask.title} denied - ${denialReason}`,
        taskId: selectedTask.id
      });
    }
    ...
    if (addNotification) {
      addNotification({
        type: 'denial',
        message: `Task "${selectedTask.title}" was denied`,
        link: 'tasks'
      });
    }
...
export default function FCProjectControl() {
  ...
  const [notifications, setNotifications] = useState([
    { id: 1, type: 'review', message: '2 tasks awaiting review', read: false, timestamp: '2026-01-09 16:45:00', link: 'review' },
    { id: 2, type: 'overdue', message: 'Load Calculations is overdue', read: false, timestamp: '2026-01-09 08:00:00', link: 'tasks' },
    { id: 3, type: 'qaqc', message: 'Beverly Center ready for QAQC', read: false, timestamp: '2026-01-09 10:00:00', link: 'projects' },
  ]);
  ...
  const addActivity = (entry) => {
    const newEntry = {
      id: Math.max(0, ...activityLog.map(a => a.id)) + 1,
  ...
  const renderView = () => {
    switch (activeView) {
      case 'dashboard':
        return <DashboardView tasks={tasks} projects={projects} activityLog={activityLog} setActiveView={setActiveView} />;
      case 'tasks':
        return <TaskQueueView tasks={tasks} setTasks={setTasks} currentUser={currentUser} projects={projects} addActivity={addActivity} addNotification={addNotification} />;
      case 'projects':
        return <ProjectsView projects={projects} setProjects={setProjects} tasks={tasks} setTasks={setTasks} addActivity={addActivity} addNotification={addNotification} currentUser={currentUser} />;
      case 'review':
        return <ReviewQueueView tasks={tasks} setTasks={setTasks} addActivity={addActivity} addNotification={addNotification} />;
      case 'activity':
        return <ActivityLogView activityLog={activityLog} />;
      case 'designer-kpi':
        return <DesignerKPIView tasks={tasks} />;
      case 'pm-kpi':
        return <PMKPIView projects={projects} activityLog={activityLog} />;
      case 'operations':
        return <OperationsView tasks={tasks} projects={projects} activityLog={activityLog} />;
      default:
        return <DashboardView tasks={tasks} projects={projects} activityLog={activityLog} setActiveView={setActiveView} />;
    }
  };
